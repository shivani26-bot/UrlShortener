  ...
  return (
    <div className="flex flex-col md:flex-row gap-5 border p-4 bg-gray-900 rounded-lg">
      ...
      <Link to={`/link/${url?.id}`} className="flex flex-col flex-1">
        <span className="text-3xl font-extrabold hover:underline cursor_pointer">
          {url?.title}
        </span>
        <span className="text-2xl text-blue-400 font-bold hover:underline cursor_pointer">
          https://trimrr.in/{url?.custom_url ? url.custom_url : url.short_url}
        </span>
        <span className="flex items-center gap-1 hover:underline cursor_pointer">
          {url?.original_url}
        </span>
